# PV: beta uses layers.csv file to read parameters for mapping - not sure whether to continue with this
library(sf)
library(leaflet)
library(tidyverse)
...
server = function(input, output, session) {
  ...
  lyr_names <- eventReactive(input$upload_poly, {
    ...
    if(ext == "gpkg"){
      x = st_layers(file)$name
      y = x[!x%in% c('studyarea','linear_disturbance','areal_disturbance','ifl_2000','ifl_2020','fires')]
    }
  })
  ...
  aoi_bnd <- eventReactive(input$upload_poly, {
    ...
    ext <- tools::file_ext(file)
    if(ext == "gpkg"){
      aoi <- st_read(file, 'studyarea', quiet=T) #%>% st_transform(4326)
    } else {
  ...
  aoi_line <- eventReactive(input$upload_poly, {
    ...
    ext <- tools::file_ext(file)
    if(ext == "gpkg"){
      aoi <- st_read(file, 'linear_disturbance', quiet=T) #%>% st_transform(4326)
    } else {
      showNotification("Wrong file type, must be geopackage (.gpkg)", type = "error")
    }
  })
  ...
  aoi_poly <- eventReactive(input$upload_poly, {
    ...
    ext <- tools::file_ext(file)
    if(ext == "gpkg"){
      aoi <- st_read(file, 'areal_disturbance', quiet=T) #%>% st_transform(4326)
    } else {
      showNotification("Wrong file type, must be geopackage (.gpkg)", type = "error")
    }
  })
  ...
  prj1 <- eventReactive(input$upload_poly, {
    ...
    if(ext == "gpkg"){
      if ('Quartz Claims' %in% st_layers(file)$name) {
        aoi <- st_read(file, 'Quartz Claims', quiet=T) %>% st_transform(4326)
      } else {
        aoi <- 'Nope'
      }
    } else {
  ...
  prj2 <- eventReactive(input$upload_poly, {
    ...
    if(ext == "gpkg"){
      if ('Placer Claims' %in% st_layers(file)$name) {
        aoi <- st_read(file, 'Placer Claims', quiet=T) %>% st_transform(4326)
      } else {
        aoi <- 'Nope'
      }
    } else {
  ...
  observeEvent(c(input$select_fda,input$upload_poly), {
    ...
    cas <- read_csv('docs/cas.csv')[42:66,2:4] %>% filter(TYPE_DISTURBANCE %in% x1)
    y <- left_join(cas, y1) %>%
  ...
  observeEvent(c(input$select_fda,input$upload_poly), {
    ...
    cas <- read_csv('docs/cas.csv')[1:41,2:4] %>% filter(TYPE_DISTURBANCE %in% x2)
    y <- left_join(cas, y2) %>%
  ...
  output$map1 <- renderLeaflet({
    ...
    m <- leaflet() %>%
      ...
      addProviderTiles("Esri.WorldTopoMap", group="Esri.WorldTopoMap")
        ...
        i2000 <- st_transform(ifl2000(), 4326)
        i2020 <- st_transform(ifl2020(), 4326)
        ...
        m <- m %>%
          ...
          addPolygons(data=i2000, fill=T, stroke=F, fillColor='#99CC99', fillOpacity=0.5, group="Intactness 2000") %>%
          addPolygons(data=i2020, fill=T, stroke=F, fillColor='#669966', fillOpacity=0.5, group="Intactness 2020") #%>%
          for (i in lyr_names()) {
            if (i %in% z$map) {
              file <- input$upload_poly$datapath
              x1 <- st_read(file, i, quiet=T) %>% st_transform(4326)
              if (z$type[z$map==i]=='poly') {
                m <- m %>% addPolygons(data=x1, fill=z$fill[z$map==i], stroke=z$stroke[z$map==i], 
                  fillColor=z$fillColor[z$map==i], fillOpacity=z$fillOpacity[z$map==i], group=i)
              } else {
                m <- m %>% addPolylines(data=x1, color=z$color[z$map==i], weight=z$weight[z$map==i], group=i)
              }
              grps <- c(grps, i)
            }
          }
  ...
  observe({
    ...
    if (input$goButton) {
      ...
      addLayersControl(position = "topright",
        baseGroups=c("Esri.WorldTopoMap", "Esri.WorldImagery"),
        overlayGroups = c("Study region", "Intactness", "Footprint", "Linear disturbances", "Areal disturbances", "Fires", "Intactness 2000", "Intactness 2020",grps),
        options = layersControlOptions(collapsed = FALSE)) %>%
        hideGroup(c("Footprint", "Fires", "Intactness 2000", "Intactness 2020",grps))
    }
  })
